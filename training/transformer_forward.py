...
# ====================================================================================
# Masking to ensure padded values are not treated as inputs: mask to identify location of zeros
# ====================================================================================
...
class Encoder(tf.keras.layers.Layer):
    def __init__(self, num_layers, d_model, num_heads, dff, input_vocab_size,
        ...
        self.embedding = tf.keras.layers.Embedding(input_vocab_size, d_model)  # input_vocab_size:80, d_model:64
        self.pos_encoding = positional_encoding(maximum_position_encoding,  # max_pos_encoding: 300, d_model:64
                                                self.d_model)
...
class MyCustomGenerator(tf.keras.utils.Sequence):
    ...
    def __init__(self, rktnt_filenames, prdct_filenames, batch_size, frac_lb_ub):
        self.rktnt_filenames = rktnt_filenames
        self.prdct_filenames = prdct_filenames
        self.batch_size = batch_size
        self.frac_lb_ub = frac_lb_ub

    def __len__(self):
        return (np.ceil(len(self.rktnt_filenames) / float(self.batch_size))).astype(np.int)

    def __getitem__(self, idx):
        # reading data batch-wise for both reactants and products
        batch_x = self.rktnt_filenames[idx * self.batch_size: (idx + 1) * self.batch_size]
        batch_y = self.prdct_filenames[idx * self.batch_size: (idx + 1) * self.batch_size]

        X, y = None, None

        for rktntfile, prdctfile in zip(batch_x, batch_y):
            if X is None:
                X, y = np.load(rktntfile)['arr_0'], np.load(prdctfile)['arr_0']
            else:
                X = np.concatenate([X, np.load(rktntfile)['arr_0']], axis=0)
                y = np.concatenate([y, np.load(prdctfile)['arr_0']], axis=0)

        inpt_orig, trgt_orig = X, y

        # split the inpt and trgt using frac: FOR GPU
        len_file = inpt_orig.shape[0]
        lb_frac, ub_frac = 0, self.frac_lb_ub
        return_res = []

        while lb_frac < 1.0:
            lb, ub = int(len_file * lb_frac), int(len_file * ub_frac)
            inpt, trgt = inpt_orig[lb:ub], trgt_orig[lb:ub]
            return_res.append((inpt, trgt))
            lb_frac, ub_frac = ub_frac, ub_frac + self.frac_lb_ub
