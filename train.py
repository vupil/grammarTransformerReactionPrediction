...
# batch_size = 3000
# epochs = 100
# FRAC_LB_UB = 0.2  # for GPU
#
# TEST_FRAC_ID = None  # only for evaluation
# TEST_FRAC = None  # only for evaluation
# BEAM_SIZE = None  # only for evaluation
# EVAL_DIR = None  # only for evaluation
#
...
num_heads = 8  # num of heads in the multi-head attention
dropout_rate = 0.1
pe_inpt, pe_targ = maxlen_products_retro+1, maxlen_products_forward  # additional token for reaction class
input_vocab_size = D+1+10  # 10 reaction classes
target_vocab_size = D+1+2

hyperparams = (
datasets_filepath, checkpoint_path, batch_size, epochs, FRAC_LB_UB, TEST_FRAC_ID, TEST_FRAC, BEAM_SIZE,
EVAL_DIR, num_layers, d_model, dff, num_heads, dropout_rate,
pe_inpt, pe_targ, NOCLASS, input_vocab_size, target_vocab_size)
...
if __name__ == "__main__":
    ...
    if MODEL == 'forward':
        ...
        LOGGER.warning('Loading/training the forward model')
        # train or load the forward model
        # WRITE WRAPPER FUNCTION

    else:
        LOGGER.error('Please provide a valid MODEL type. Exiting..')
